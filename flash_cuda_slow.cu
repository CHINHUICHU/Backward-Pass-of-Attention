...
__global__ void rowwise_backward_kernel(
    ...
    for (int j = 0; j < n; j++) {
        ...
        // Eq (4): gamma_i = rowsum(dO_{i,:} ⊙ O_{i,:})
        float gamma_i = 0.0f;
        for (int x = 0; x < D; x++) {
...
__global__ void naive_nodupe_backward_kernel(
    ...
    for (int j = 0; j < n; j++) {
        ...
        float dS_ij = P_ij * (dP_ij - Delta[i]) * scale;
...
int main() {
    ...
    for (int t = 0; t < num_tests; t++) {
        ...
        float ms_naive=0, ms_uncoal=0, ms_flash=0, ms_rowwise=0, ms_nodupe=0;
        ...
        flush_l2_cache_kernel<<<flush_size/256, 256>>>(d_flush, flush_size);
        cudaEventRecord(start);
        naive_nodupe_backward_kernel<<<n/256+1, 256>>>(d_Q, d_K, d_V, d_P, d_dO, d_Delta, d_dQ, d_dK, d_dV, n, D, scale);
        cudaEventRecord(stop); cudaEventSynchronize(stop);
        cudaEventElapsedTime(&ms_nodupe, start, stop);

        printf("| %-8d | %-12.2f | %-12.2f | %-12.2f | %-12.2f | %-12.2f |\n", n, ms_naive, ms_nodupe, ms_uncoal, ms_flash, ms_rowwise);
